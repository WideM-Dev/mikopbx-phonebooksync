...
├── Lib/
│   ├── ModulePhoneBookConf.php        # Main ConfigClass (routing & auth)
│   └── ApiController.php              # REST API endpoints
├── Models/
│   └── PhoneBookContact.php           # Phalcon model — external contacts
├── Messages/
...
All endpoints under `/pbxcore/api/modules/ModulePhoneBook/`:
...
After every add/edit/delete the module automatically writes to MikoPBX's built-in `pb_PhoneBook` table. Entries are tagged with `source = 'ModulePhoneBook'` and cleaned up on uninstall.
...
### v1.1.4
- Added `ModulePhoneBookConf` ConfigClass for MikoPBX routing
...
### v1.1.1
- Added `BreadcrumbModulePhoneBook` translation key (fixes module name in UI)
